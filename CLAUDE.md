...
一套**跨 AI 平台**的多角色设计协作框架。**6 主角色 + 1 可选角色**按需接力，把模糊需求推进到可上线方案。

设计阶段 5 个角色（01-05）+ 上线后质量循环 1 个角色（06）+ 可选上线运营助手 1 个（07），全部脚本在 `workflow/roles/` 目录。
...
可选支线（增强模式，需要对外传播 / 用户教育 / 增长时启用）：
05 Reviewer 完成后 → 07 Launch Ops（用户显式触发 /dw-launch，不自动接管 current_role）
```
